...
public class ProcessPostAutoApprovalActor : BaseActor
{
    private readonly IServiceScopeFactory _serviceScopeFactory;

    public ProcessPostAutoApprovalActor(IServiceScopeFactory serviceScopeFactory)
    {
        _serviceScopeFactory = serviceScopeFactory;

        ReceiveAsync<ProcessPostAutoApprovalMessage>(DoProcessPostAutoApproval);
    }
    ...
    private async Task DoProcessPostAutoApproval(ProcessPostAutoApprovalMessage message)
    {
        using var scope = _serviceScopeFactory.CreateScope();
        var service = scope.ServiceProvider.GetRequiredService<IPostAutoApprovalService>();
        var res = await service.ApproveAsync(message.Id);
        ...
        if (failed && shouldRetry && taskConfig.RetryCount < res.MaxRetries)
            await ReSchedulerService.Reschedule<PostAutoApprovalJobHandler>(message.JobExecutionContext);
    }
}